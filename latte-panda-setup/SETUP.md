...
A copy of the NxSDK 1.0.0 is available in this project director at `nxsdk/`  
You need to install the following tarballs:  
`nxsdk.1.0.0.tar.gz`   
`nxsdk-apps-1.0.0.tar.gz` 
First turn on permissions to execute these but don't install them yet:    
```
chmod 777 nxsdk-1.0.0.tar.gz

chmod 777 nxsdk-apps-1.0.0.tar.gz
```
In the NxSDK folder, there is also a file named “nxsdk-docs-v095.exe”. When executed, it opens Intel’s NxSDK documentation, which contains useful information about programming the Loihi.
...
- Do all of the following steps within the virtual environment. You know that your VE is activate when your terminal prompt changes - it should have the prefix of your VE name (i.e. "loihi")
    ...
    - Go to the `nxsdk/` directory and install the nxsdk (this may take up to 10 minutes to complete): 
    ```bash
    cd ~
...
the source code found in this repo. The recommended next step, is to check out the NxSDK documentation and tutorials, then 
move onto the tutorials found in this project. 

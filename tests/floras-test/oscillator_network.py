"""
Creates a multi-layer network for Arduino-Loihi pipeline.
Each layer is connected to the previous layer with one-to-one connections.

Adapted from https://github.com/michaelgzt/loihi-control-loop-demo/blob/main/loihi_network.py
...
class LoihiNetwork:
    """Defines a multi-layer SNN with sequential connections"""

    def __init__(self, layer_dim, core_lists):
        """
        Args:
            layer_dim (int): layer dimension
            core_lists (list): neuron core lists
        """
        self.net = nx.NxNet()
        self.layer_dim = layer_dim
        self.core_lists = core_lists
    ...
    def setup_loihi_snn(self, encode_dim, decode_dim, snip_dir, print_axon=False):
        ...
        Args:
            encode_dim (int): encoder channel dimension
            decode_dim (int): decoder channel dimension
        ...
        """
        # Setup layers
        layers = [self.set_neuron_layer(self.net, self.core_lists[i]) for i in range(NUM_LAYERS)]
        
        fake_input_conn = self.set_input_encoding(self.net, self.layer_dim, layers[0], self.core_lists[0])
        self.set_output_decoding(layers[len(layers)-1])

        # Setup connections between layers
        for i in range(NUM_LAYERS-1):
            self.set_neuron_connection(self.layer_dim, layers[i], layers[i+1])

        # Compile Network
        compiler = nx.N2Compiler()
        board = compiler.compile(self.net)

        # Print axon information
        if print_axon:
